...
/******************************************************************************
*  Supply a file named vec.txt with the position vector in units of earth     *
*  radii and the velocity vector in units of earth radii / minute.  Run the   *
...
int main(int argc, char *argv[])
{
  ...
  {
    sprintf(file_in, "VEC.txt");
    sprintf(file_out, "VEC.txt");
  }
...
void get_vec(char *file_in)
{
  ...
  {
    fgets(buf, 20, fp);
    if(i < 3)  rr[i] = atof(buf);
    if(i >= 3)  vv[i - 3] = atof(buf);
  }
  fclose(fp);
}
